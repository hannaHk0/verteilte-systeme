...
async function updateKurs(kursId, updateData) {
  const db = getDB();
  await db.read();
  ...
  const updatedKurs = {
    ...db.data.kurs[index],
    ...updateData,
    id: db.data.kurs[index].id,         
    kursId: db.data.kurs[index].kursId  
  };
  
  db.data.kurs[index] = updatedKurs;
  await db.write();
  
  
  const topic = `WWI23B3_Huber/Kurs`; 
  const message = JSON.stringify({
    event: 'kursAktualisiert',
    url: `/kurs/${kursId}`,
    data: updatedKurs
  });
  publishMessage(topic, message);
...
export { createKurs, getKurs, updateKurs, deleteKurs };